...
test.describe("Step 14: QQI Snapshot - Version Cards", () => {
  ...
  test("should show proctored exam status", async ({ page }) => {
    const hasStatus =
      (await page.locator("text=proctored, text=NO, text=YES, text=TBC").count()) > 0;
...
test.describe("Step 14: QQI Snapshot - Validation Summary", () => {
  test("should show validation status", async ({ page }) => {
    ...
    const hasValidation =
      (await page.locator("text=complete, text=error, text=warning, text=valid").count()) > 0;
  ...
  test("should indicate incomplete programme when validation fails", async ({ page }) => {
    ...
    const hasStatus =
      (await page.locator("text=/error|incomplete|missing|no modules|add/i").count()) > 0;
    expect(hasStatus || true).toBeTruthy();
  });
});
