...
import { getAwardStandard, getStandardIndicators, state } from "../../state/store.js";
import { escapeHtml } from "../../utils/dom.js";
import { mimloText, ploText } from "../../utils/helpers.js";
...
function buildTraceRows(p, standardsDataArray) {
  ...
  if (standardsDataArray.length > 0 && nfqLevel) {
    const coverageCards = standardsDataArray
      .map(({ id, data }) => {
        const levelStandards = data?.levels?.[nfqLevel] ?? [];
        const covered = coveredStandardsByAward.get(id) ?? new Set();
...
function buildTableRowsHtml(traceRows, hasMultipleStandards) {
  ...
  const statusBadge = (status, label) => {
    if (status === "ok") {
      return `<span class="badge text-bg-success">${escapeHtml(label)}</span>`;
    }
    if (status === "warning") {
...
function wireTraceability() {
  const filterStatus = /** @type {HTMLSelectElement | null} */ (
    document.getElementById("traceFilterStatus")
  );
  const filterModule = /** @type {HTMLSelectElement | null} */ (
    document.getElementById("traceFilterModule")
  );
  ...
  function applyFilters() {
    ...
    table.querySelectorAll("tbody tr").forEach((row) => {
      ...
      if (moduleVal !== "all" && rowModule !== moduleVal) {
        show = false;
      }
  ...
  if (exportBtn && table) {
    exportBtn.onclick = () => {
      ...
      table.querySelectorAll("tbody tr").forEach((tr) => {
        if (/** @type {HTMLElement} */ (tr).style.display === "none") {
          return;
        }
...
function buildSankeyData(traceRows, hasMultipleStandards) {
  ...
  traceRows.forEach((/** @type {any} */ row) => {
    ...
    const standardPrefix =
      hasMultipleStandards && row.awardStandardId ? `[${row.awardStandardId}] ` : "";
    ...
    if (row.status === "uncovered") {
      ...
      if (standardNode !== undefined && gapNode !== undefined) {
    ...
    if (row.ploNum !== "—") {
      ...
      if (standardNode !== undefined && ploNode !== undefined) {
      ...
      if (row.moduleCode && row.moduleCode !== "—") {
        ...
        if (ploNode !== undefined && moduleNode !== undefined) {
        ...
        if (row.mimloNum !== "—") {
          ...
          if (moduleNode !== undefined && mimloNode !== undefined) {
          ...
          if (row.assessmentTitle && row.status === "ok") {
            ...
            if (mimloNode !== undefined && asmNode !== undefined) {
...
function renderSankeyDiagram(sankeyData) {
  ...
  if (!container || !(/** @type {any} */ (window).Plotly)) {
    return;
  }
  ...
  if (sankeyData.links.source.length === 0) {
    container.innerHTML =
      '<div class="alert alert-info">No alignment data to visualize. Add mappings between PLOs, modules, and assessments.</div>';
    return;
  }
