...
export function validateProgramme(p) {
  ...
  const flags = [];
  const sumCredits = (p.modules ?? []).reduce((/** @type {number} */ acc, /** @type {Module} */ m) => acc + (Number(m.credits) ?? 0), 0);
  ...
  const mandatoryModules = (p.modules ?? []).filter((/** @type {Module & {isElective?: boolean}} */ m) => !m.isElective);
  const mandatoryCredits = mandatoryModules.reduce((/** @type {number} */ acc, /** @type {Module} */ m) => acc + (Number(m.credits) ?? 0), 0);
  ...
  if (electiveDefinitions.length === 0) {
    ...
    electiveDefinitions.forEach((/** @type {ElectiveDefinition} */ def, /** @type {number} */ defIdx) => {
      ...
      groups.forEach((/** @type {ElectiveGroup} */ g, /** @type {number} */ grpIdx) => {
        ...
        if (!g.moduleIds || g.moduleIds.length === 0) {
          ...
          const groupModules = (p.modules ?? []).filter((/** @type {Module} */ m) => g.moduleIds.includes(m.id));
          const nonElectiveInGroup = groupModules.filter((/** @type {Module & {isElective?: boolean}} */ m) => !m.isElective);
          ...
          const groupCreditsSum = groupModules.reduce((/** @type {number} */ acc, /** @type {Module} */ m) => acc + (Number(m.credits) ?? 0), 0);
    ...
    moduleGroupAssignments.forEach((assignments, moduleId) => {
      if (assignments.length > 1) {
        ...
        const groupNames = assignments.map((/** @type {{groupId: string, groupName: string}} */ a) => a.groupName).join(', ');
    ...
    const definitionCreditsSum = electiveDefinitions.reduce((/** @type {number} */ acc, /** @type {ElectiveDefinition} */ def) => acc + (Number(def.credits) ?? 0), 0);
  ...
  if (!Array.isArray(p.versions) || p.versions.length === 0) {
    ...
    p.versions.forEach((/** @type {ProgrammeVersion} */ v, /** @type {number} */ idx) => {
      ...
      if ((/** @type {any} */ (v).onlineProctoredExams ?? "TBC") === "YES" && !(/** @type {any} */ (v).onlineProctoredExamsNotes ?? "").trim()) {
        flags.push({ type: "warn", msg: `${prefix}: online proctored exams marked YES but notes are empty.`, step: "versions" });
      }

      if ((/** @type {any} */ (v).targetCohortSize ?? 0) <= 0) flags.push({ type: "warn", msg: `${prefix}: cohort size is missing/zero.`, step: "versions" });

      // Stage structure
      if (!Array.isArray(v.stages) || v.stages.length === 0) {
        flags.push({ type: "warn", msg: `${prefix}: no stages defined yet.`, step: "stages" });
      } else {
        const stageTargetSum = (/** @type {any[]} */ (v.stages) ?? []).reduce((/** @type {number} */ acc, /** @type {any} */ s) => acc + Number(s.creditsTarget ?? 0), 0);
        ...
        (/** @type {any[]} */ (v.stages) ?? []).forEach((/** @type {any} */ s) => {
          ...
          const creditSum = (p.modules ?? [])
            ...
            .reduce((/** @type {number} */ acc, /** @type {Module} */ m) => acc + Number(m.credits ?? 0), 0);
  ...
  const modulesMissingMimlos = (p.modules ?? []).filter((/** @type {Module & {mimlos?: any[]}} */ m) => !m.mimlos || m.mimlos.length === 0);
  ...
  const unmappedPLOs = (p.plos ?? []).filter((/** @type {PLO} */ o) => !(p.ploToModules ?? {})[o.id] || ((p.ploToModules ?? {})[o.id] ?? []).length === 0);
